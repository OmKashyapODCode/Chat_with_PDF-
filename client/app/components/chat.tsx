...
const ChatComponent: React.FC = () => {
  ...
  const handleSendChatMessage = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/chat?message=${encodeURIComponent(userMessage)}`);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 scroll-smooth">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-4">
            ...
            <p className="text-sm font-medium">Hello! Ask me anything about your PDF.</p>
          </div>
        )}
          ...
          return (
            <div key={index} className={`flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'} items-end`}>
              ...
              <div className={`flex flex-col max-w-[85%] md:max-w-[75%] ${isUser ? 'items-end' : 'items-start'}`}>
                ...
                {/* PDF Source Citations (Only show for assistant) */}
                {!isUser && msg.documents && msg.documents.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1.5 ml-1">
                    {msg.documents.map((doc, docIdx) => {
                      ...
                      return (
                        <span key={docIdx} className="text-[10px] px-2.5 py-1 rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 flex items-center shadow-sm">
                          📄 Pg {page}
                        </span>
                      );
                    })}
